...
public interface PagingIterable<ElementT> extends Iterable<ElementT> {
   ...
   *
   * <p>In theory this method should not be exposed on {@link PagingIterable} since it is specific
   * to CQL; in practice, most {@code PagingIterable} instances will be created by wrapping a {@link
   * ResultSet}, so it is convenient to make this information available without having to unwrap.
   *
